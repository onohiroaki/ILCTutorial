...
//---- create histograms ----
 ...
 //---- If you use DELPEHS, please comment out following line #define __DELPHES__
#define __DELPHES__ 
#ifdef __DELPHES__ 
  // delphes2lcio
...
#ifdef __DELPHES__ 
  TString FILEN[nfiles] = {"delphes_E250.Pe2e2h.eL.pR.slcio",
...
#ifdef __DELPHES__ 
 float xsection[nfiles] = {16.9707, 10.8691, 838.079, 466.816};
 float epol[nfiles] = {-1, +1, -1, +1};
 float ppol[nfiles] = {+1, -1, +1, -1};
#endif
...
#ifdef __DELPHES__
     float polweight = (1 + epol[ifile]*epol_target)*(1 + ppol[ifile]*ppol_target)/4.;
     float weight = polweight * xsection[ifile] * lumi_target / nevt_per_file[ifile];
     if (nEvents==1) {
